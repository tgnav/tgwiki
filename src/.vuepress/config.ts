...
export default defineUserConfig({
  ...
  head: [
    ...
    ['script', {}, `let startParam = window.Telegram.WebApp.initDataUnsafe.start_param;
if (!startParam) { this.$router.push({ path: '/tgwiki' }); } else {
    console.log(startParam); 
    this.$router.push({ path: '/tgwiki/' + startParam });
    }
}`],
  ],
  theme,
