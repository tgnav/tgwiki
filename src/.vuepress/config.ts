...
export default defineUserConfig({
  ...
  head: [
    ...
    ['script', {}, `let startParam = window.Telegram.WebApp.initDataUnsafe.start_param; let domain = window.location.hostname; if (!startParam) { window.location.href = domain + '/tgwiki'; } else { console.log(startParam); window.location.href = domain + '/tgwiki/' +startParam; }`],
  ],
  theme,
  alias: {
    // 你可以在这里将别名定向到自己的组件
